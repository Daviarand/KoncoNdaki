...
// Data Dashboard
$stmtKpi = $pdo->query("SELECT (SELECT COUNT(*) FROM users WHERE role = 'pendaki') as total_pendaki, (SELECT COUNT(*) FROM pemesanan WHERE status_pemesanan IN ('pending', 'menunggu pembayaran')) as total_booking_baru, (SELECT SUM(total_harga) FROM pemesanan WHERE status_pemesanan = 'complete' AND MONTH(tanggal_pemesanan) = MONTH(CURDATE())) as total_pendapatan_bulan_ini, (SELECT g.nama_gunung FROM pemesanan p JOIN gunung g ON p.tiket_id = g.id GROUP BY g.nama_gunung ORDER BY COUNT(p.id) DESC LIMIT 1) as gunung_populer");
$kpi = $stmtKpi->fetch(PDO::FETCH_ASSOC);
$stmtPendapatanGunung = $pdo->query("SELECT g.nama_gunung, SUM(p.total_harga) as pendapatan FROM pemesanan p JOIN gunung g ON p.tiket_id = g.id WHERE p.status_pemesanan = 'complete' AND MONTH(p.tanggal_pemesanan) = MONTH(CURDATE()) GROUP BY g.nama_gunung ORDER BY pendapatan DESC");
...
$baseSqlLaporan = " FROM pemesanan p JOIN gunung g ON p.tiket_id = g.id WHERE p.status_pemesanan = 'complete'";
$params = [];
$filterSql = "";
        ...
        <main class="main-content">
            ...
            <section id="dashboard" class="section">
                <div class="section-header">
                    <h2>Ringkasan Seluruh Gunung</h2>
                </div>
            ...
            <section id="laporan" class="section">
                <div class="section-header">
                    <h2>Laporan & Analisis Lintas Gunung</h2>
                </div>
                        ...
                        <table class="data-table">
                            ...
                            <tbody>
                                <?php if (empty($daftarPengelola)): ?>
                                    <tr>
                                        <td colspan="4" style="text-align: center;">Belum ada akun pengelola.</td>
                                    </tr>
                                <?php else: ?>
                        ...
                        <table class="data-table">
                            ...
                            <tbody>
                                <?php if (empty($daftarPendaki)): ?>
                                    <tr>
                                        <td colspan="4" style="text-align: center;">Belum ada user pendaki.</td>
                                    </tr>
                                <?php else: ?>
            ...
            <section id="tambahPengelola" class="section">
                ...
                <div class="form-container">
                    ...
                    <form method="POST" action="admin.php#tambahPengelola" id="create-user-form">
                        ...
                        <div class="form-group"><label for="gunung_id">Tugaskan ke Gunung</label><select id="gunung_id" name="gunung_id" required>
                                <option value="" disabled selected>-- Pilih Gunung --</option><?php $stmtGunungListForm = $pdo->query("SELECT id, nama_gunung FROM gunung WHERE admin_id IS NULL");
        ...
        document.addEventListener('DOMContentLoaded', function() {
                ...
                new Chart(document.getElementById('pendapatanChart').getContext('2d'), {
                    ...
                    data: {
                        ...
                        datasets: [{
                            ...
                            backgroundColor: 'rgba(22, 163, 74, 0.8)', // Slightly more opaque green
                            borderColor: 'rgba(22, 163, 74, 1)',
                            borderWidth: 1,
                            borderRadius: 5, // Rounded bars
                            barPercentage: 0.7 // Make bars a bit wider
                        }]
                    },
                    options: {
                        responsive: true,
            ...
            if (document.getElementById('roleChart')) {
                new Chart(document.getElementById('roleChart').getContext('2d'), {
                    type: 'doughnut',
                    data: {
                        labels: <?php echo json_encode(array_keys($roleData)); ?>,
                        datasets: [{
                            label: 'Jumlah Pengguna',
                            data: <?php echo json_encode(array_values($roleData)); ?>,
                            backgroundColor: ['#34d399', '#60a5fa', '#facc15', '#a78bfa', '#f87171', '#fb923c']
                        }]
                    },
                    options: {
                        responsive: true,
            ...
            if (document.getElementById('analisisPendapatanChart')) {
                new Chart(document.getElementById('analisisPendapatanChart').getContext('2d'), {
                    type: 'doughnut',
                    data: {
                        labels: ['Pendapatan Tiket', 'Pendapatan Layanan'],
                        datasets: [{
                            data: [<?php echo $analisisPendapatan['total_tiket'] ?? 0; ?>, <?php echo $analisisPendapatan['total_layanan'] ?? 0; ?>],
